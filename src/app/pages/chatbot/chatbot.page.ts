...
export class ChatbotPage implements OnInit {
  ...
  async functionReturnData(){
    ...
    if(this.seconds<10) 
      this.seconds = '0'+ this.seconds;
    this.genaratedFullDate = this.year+""+this.month+""+this.day+this.hour+this.minutes+this.seconds;
    let sendValues = {'mainUserName':this.mainUserName,'userName':this.userName,'password':this.password,'apiKey':this.apiKey,'onliceData':2,'dateSelect':this.genaratedFullDate,'sessionLogin':this.sessionLogin,'mobileSelect':'962785086985'};
    this.chatService.chatGetData(sendValues).then(async data=>{
      ...
      if(errorData == 1){
        ...
        if(typeof this.returnResultData.data.bot!== 'undefined'){
          let counter = 0;
          let countOfData = 0;
          this.returnArrayChatFromServer = this.returnResultData.data.bot;
          for(let i = 0; i < this.returnArrayChatFromServer.length;i++) {
            if(this.returnArrayChatFromServer[i].userName == "bot"){
              this.returnChatArray[counter]=[];
              this.returnChatArray[counter]['mobile'] = this.returnArrayChatFromServer[i].mobile;
              ...
              this.returnChatArray[counter]['badge'] = this.returnArrayChatFromServer[i].badge;
  ...
  functionChats(number:any,chatSessionId:any,userName:any){
    this.navCtrl.navigateRoot(['/chats', {number:number,chatSessionId:chatSessionId,userNameUsed:userName,backUrl:2}]);
  }
